...
import wandb
from transforms import train_transforms, val_transforms, shape_transforms
...
train_dataset = UKB(data_path, masks_path, train=True, val_split=0.2,
       shape_transforms=shape_transforms, transforms=train_transforms)
val_dataset = UKB(data_path, masks_path, train=False, val_split=0.2,
       shape_transforms=shape_transforms, transforms=val_transforms)
# use a sampler to make batches of only 1000 subjects
sampler = torch.utils.data.RandomSampler(train_dataset, num_samples=100,\
       replacement=True)
train_loader = DataLoader(train_dataset, batch_size=config["batch_size"],
       sampler=sampler, num_workers=24)
# train_loader = DataLoader(train_dataset, batch_size=config["batch_size"],
...
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,
       config["epochs"])
trainer = Trainer(model, train_dataset, train_loader, val_loader, criterion,
